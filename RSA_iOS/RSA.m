...
{
    // NSLog(@"%@",longStr);
    NSMutableString * stringR =[[NSMutableString alloc]init];
    
    for(int i=0;i<((longStr.length - longStr.length%117)/117+1); ++i)
    {
        NSString * resultString = [[NSString alloc]init];
        
        if (i <(longStr.length - longStr.length%117)/117) {
            resultString = [longStr substringWithRange:NSMakeRange(i*117,117)];
        }else
        {
            resultString= [longStr substringWithRange:NSMakeRange(i*117,longStr.length-i*117)];
        }
        NSString *  resultRSA = [NSString stringWithFormat:@"%@",[RSA encryptString:resultString publicKey:pubKey]];
        [stringR appendString:resultRSA];
    }
